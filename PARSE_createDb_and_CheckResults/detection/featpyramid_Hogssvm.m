function pyra = featpyramid_Hogssvm(im, model)
...
max_scale = interval;%1 + floor(log(min(imsize)/(5*sbin))/log(sc));
...
for i = 1:interval
  ...
  fprintf('pyra.feat{%d} size [%d %d], sbin %d\n',i,size(pyra.feat{i},1),size(pyra.feat{i},2),sbin);
  % remaining interals
%   for j = i+interval:interval:max_scale
%     scaled = reduce(scaled);
%     pyra.feat{j} = features(scaled,sbin);
%     pyra.scales(j) = 0.5 * pyra.scales(j-interval);
%   end
end
...
pyra.scales    = model.sbin./pyra.scales;
